...
if a == 1:
    ...
    y1arr = np.array(np.cos(x) + 1.2)
    y2arr = np.array(-0.5 * x ** 2 + 0.7)
    ...
    while c != 1000:
        ...
        s2 += (y2(i + 4 / 1000) + y2(i)) / 2 * np.pi / 1000
        i += np.pi / 1000
        c += 1
    ...
    y1arr = np.array(np.e ** (-x ** 2) + 0.5)
    y2arr = np.array(0.2 * np.sin(3 * x) - 0.5)
    ...
    def y1(x):
        return np.e ** (-(x - 1) ** 2) + 0.5

    def y2(x):
        return -0.3 * x ** 2

    y1arr = np.array(np.e ** (-(x - 1) ** 2) + 0.5)
    y2arr = np.array(-0.3 * x ** 2)
